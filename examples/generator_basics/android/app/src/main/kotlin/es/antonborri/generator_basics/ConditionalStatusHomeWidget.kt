    ...
    companion object {
        ...
        fun fromPreferences(prefs: android.content.SharedPreferences): ConditionalStatusData {
            return ConditionalStatusData(
                hasData = if (prefs.contains("${PREFERENCES_PREFIX}.hasData")) prefs.getBoolean("${PREFERENCES_PREFIX}.hasData", false) else null,
                enabled = if (prefs.contains("${PREFERENCES_PREFIX}.enabled")) prefs.getBoolean("${PREFERENCES_PREFIX}.enabled", false) else true,
            )
        }
    }
}
