    ...
    GlanceTheme {
            Box(modifier = GlanceModifier.background(GlanceTheme.colors.widgetBackground).padding(16.dp).fillMaxSize(), contentAlignment = Alignment.Center) {
                Column(horizontalAlignment = Alignment.Start) {
                    Text(text = context.getString(R.string.home_widget_localized_greeting_t_1e28f816), style = TextStyle(fontSize = 12.sp, fontWeight = FontWeight.Normal))
                    Text(text = widgetData.greeting ?: "", style = TextStyle(fontSize = 22.sp, fontWeight = FontWeight.Bold))
                }
            }
    }
...
data class LocalizedGreetingData(
    ...
    companion object {
        ...
        fun fromPreferences(prefs: android.content.SharedPreferences, locales: List<String>): LocalizedGreetingData {
            return LocalizedGreetingData(
                greeting = hwReadLocalized(prefs, "${PREFERENCES_PREFIX}.greeting", locales, mapOf("en" to "Hello", "de" to "Hallo", "pt-BR" to "Olá"), "en"),
            )
        }
    }
}
