...
Android widgets can be made configurable by adding the following fields to your widget XML's `<appwidget-provider>` element, where
`com.example.app` is your app's package name:

```
android:widgetFeatures="reconfigurable"
android:configure="com.example.app.MainActivity"
```

This will enable the "Settings" option in the menu when you long-press the widget.

You will also need to add an intent filter to the `<activity>` element in AndroidManifest.xml:
...
You can then detect whether the app was launched from the widget settings menu item by calling
`HomeWidget.initiallyLaunchedFromHomeWidgetConfigure()`. If the app was launched by a widget configuration request, the returned `Future`
will resolve to the corresponding widget ID (as a string). Otherwise, it will resolve to `null`. If a widget configuration request was
detected, this method will also set the default Activity result to `RESULT_CANCELED`.

Once the widget configuration is saved, call `HomeWidget.finishHomeWidgetConfigure()`. This will end the widget configuration activity
and return `RESULT_OK`.

For more information, see the [Android documentation](https://developer.android.com/develop/ui/views/appwidgets/configuration).