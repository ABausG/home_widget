...
<Info>
The [generator](/generator) can build Lock Screen widgets for you: give an [`HWSizeAdaptive`](/generator/widgets/hw-size-adaptive) an `accessoryCircular`, `accessoryRectangular` or `accessoryInline` slot and list those families in `supportedFamilies`.
</Info>
