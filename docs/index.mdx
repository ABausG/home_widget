...
HomeWidget does **not** allow writing widgets with Flutter itself. You can either write the widgets with native code, or describe them once in Dart with [`home_widget_generator`](/generator) and generate the native sources with [`home_widget_cli`](/cli). Either way, `home_widget` provides a unified interface for sending data, retrieving data, and updating the widgets.
...
- [Generator](/generator/getting-started) - Describe a widget in Dart and generate native iOS and Android code with `home_widget_generator` and `home_widget_cli`
