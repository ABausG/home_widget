...
To run the generator, use the companion [`home_widget_cli`](/cli) tool. It discovers your annotated schemas, runs `build_runner` under the hood, and writes the generated files into your Flutter project.

## What gets generated

From a single annotated Dart class, the generator produces:

- A typed **Dart helper** (e.g. `MyWidgetHomeWidget`) with `ensureInitialized()`, an `updateWidget()` method, and `setData(...)` / `update(...)` methods for any data fields you declared.
...
- [Widgets](/generator/widgets/hw-adaptive) - the layout primitives (`HWColumn`, `HWText`, `HWImage`, ...) you use to describe the UI.
