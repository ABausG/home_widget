...
- A typed **Dart helper** (e.g. `MyWidgetHomeWidget`) with `saveData(...)`, `getData()`, `deleteData(...)`, and `updateWidget()` methods, plus pinning (`requestPinWidget()`), install status (`isInstalled()`, `getInstalledWidgets()`), and a gallery preview refresh (`updatePreview()`) scoped to that one widget.
