...
- A typed **Dart helper** (e.g. `MyWidgetHomeWidget`) with `saveData(...)`, `getData()`, `deleteData(...)`, and `updateWidget()` methods, plus install status (`isInstalled()`, `getInstalledWidgets()`) scoped to that one widget. A widget with an Android configuration additionally gets pinning (`requestPinWidget()`) and a gallery preview refresh (`updatePreview()`) — both are Android features, so an iOS-only widget gets neither.
...
- [Formatting](/generator/formatting) - locale-aware numbers, currencies, percentages and dates, formatted on the device.
