...
- A typed **Dart helper** (e.g. `MyWidgetHomeWidget`) with `saveData(...)`, `getData()`, `deleteData(...)`, and `updateWidget()` methods, plus pinning (`requestPinWidget()`) and install status (`isInstalled()`, `getInstalledWidgets()`) scoped to that one widget.
