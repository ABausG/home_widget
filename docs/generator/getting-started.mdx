...
<Steps>
  <Step title="Add dependencies">
    ...
    dev_dependencies:
      home_widget_generator: ^0.1.0
      home_widget_cli: ^0.1.0
    ```
  </Step>
  ...
  <Step title="Add data and styling">
    ...
    Every `HWString` / `HWInt` / `HWBool` / `HWDouble` / `HWDateTime` you reference inside `widget:` becomes a typed field on the generated helper, so you can update it without juggling string keys. Numbers and dates are rendered in the device's own locale — see [Formatting](/generator/formatting).
  </Step>
  ...
  <Step title="Use the generated Dart helper">
    ...
    - `requestPinWidget()` / `isRequestPinWidgetSupported()` ask the launcher to place this widget on the home screen (Android only).
    - `getInstalledWidgets()` / `isInstalled()` report where this widget is currently placed.

    See [Sync data](/usage/sync-data) for the underlying `home_widget` behavior, and [Pinning & Install Status](/generator/install-status) for the last two groups.
  </Step>
  ...
  <Step title="Native setup">
    **No manual native setup is needed**: the CLI registers the widget receiver in `AndroidManifest.xml`, adds the iOS widget extension to the Xcode project, writes the entitlements file, and bumps the deployment target as required. You only need an App Group ID configured for iOS — provide it via the `iOS: HomeWidgetIOSConfiguration(groupId: ...)` field on the annotation.
    ...
    The Xcode project is the `.xcodeproj` in `ios/` — `Runner.xcodeproj` unless you renamed it — and the widget is attached to its app target. If `ios/` holds no Xcode project, the CLI stops before writing anything.
  </Step>
</Steps>
