...
Renders `whenPresent` if the data field has a value in the widget storage, otherwise `whenAbsent`. Works with any [`HWDataType`](/generator/data). For an [`HWImageData`](/generator/widgets/hw-image) it also checks that the saved picture is still there, so the `whenAbsent` placeholder shows instead of an empty gap.
